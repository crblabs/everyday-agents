...
**I use ChatGPT/Codex, Grok, or Gemini — can I still use this?**
Partly today, maybe fully soon — honest answer in two halves. The kit's *content* is portable on purpose: the instruction sheets, the safety rules, and the guided flows are plain files any capable AI agent can read (there's even an [AGENTS.md](../AGENTS.md) that orients non-Claude agents). The *scheduled-in-the-cloud* part is what differs per platform — Codex has "automations", Grok has bot routines — and the kit has only been **verified end-to-end on Claude**. [The runtimes map](runtimes.md) says exactly what's known, what's not, and how to become the person who verifies your platform (which the community would celebrate).
